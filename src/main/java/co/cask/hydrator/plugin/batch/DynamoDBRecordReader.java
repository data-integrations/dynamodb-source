...
public class DynamoDBRecordReader extends RecordReader<LongWritable, Item> {
  ...
  private DynamoDB getDynamoDbClient(Configuration conf) {
    ...
    if (Strings.isNullOrEmpty(conf.get(DynamoDBConstants.REGION))) {
      dynamoDBClient.withRegion(Regions.DEFAULT_REGION);
    } else {
      dynamoDBClient.withRegion(RegionUtils.getRegion(conf.get(DynamoDBConstants.REGION)));
    }

    if (!Strings.isNullOrEmpty(conf.get(DynamoDBConstants.ENDPOINT_URL))) {
      dynamoDBClient.setEndpoint(conf.get(DynamoDBConstants.ENDPOINT_URL));
    }
  ...
  private void testTableExists(DynamoDB dynamoDB) {
    TableDescription tableDescription = dynamoDB.getTable(conf.get(DynamoDBConstants.TABLE_NAME)).describe();
    if (!tableDescription.getTableStatus().equals(TableStatus.ACTIVE.name())) {
      throw new IllegalArgumentException(
        String.format("Table '%s' does not exist. Please check that the table exists or specified correctly.",
                      conf.get(DynamoDBConstants.TABLE_NAME)));
    }
  }
}
